...
Example
-------
Run a quick 60s sanity check on the second recording::

    cd P4_EEG降噪与任务特征保真/experiment/video
    python -m analysis.run_analysis \\
        --video scratch/video_records/camera_20260518_193936.mp4 \\
        --duration 60 \\
        --save-preview

Outputs land in ``scratch/video_records/analysis_outputs/<video_stem>/``
...
def _default_output_dir(video_path: Path) -> Path:
    return video_path.parent / "analysis_outputs" / video_path.stem
