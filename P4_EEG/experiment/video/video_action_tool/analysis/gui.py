...
SCRIPT_DIR = Path(__file__).resolve().parent          # video/video_action_tool/analysis/
VIDEO_DIR = SCRIPT_DIR.parent.parent                  # experiment/video/
EXPERIMENT_DIR = VIDEO_DIR.parent                     # experiment/
DEFAULT_RECORDS_DIR = EXPERIMENT_DIR / "data" / "video_records"
DEFAULT_OUTPUT_PARENT = VIDEO_DIR / "analysis_outputs"
FACE_MODEL_URL = (
...
class VideoActionGUI:
    ...
    def _set_default_paths(self) -> None:
        ...
        if videos:
            ...
            self.output_dir_var.set(str(DEFAULT_OUTPUT_PARENT))
    ...
    def _auto_output_dir(self, video_path: Path) -> None:
        # 视频统一放在 experiment/data/video_records/，分析产物落在 experiment/video/analysis_outputs/<stem>/，
        # 与 data 目录读写分离。
        self.output_dir_var.set(str(DEFAULT_OUTPUT_PARENT / video_path.stem))
