...
# Quick-test (冒烟测试) 参数：保留 75/25 比例与连续靶刺激≤2 的约束
QUICK_N_TOTAL = 8
QUICK_TARGET_RATIO = 0.25
QUICK_PRE_STIM_QUIET = 0.05
QUICK_ITI_MIN = 0.05
QUICK_ITI_MAX = 0.10
QUICK_BEHAVIOR_CHECK_INTERVAL = 100  # 大于 N_TOTAL, 实际不会触发
...
def run_oddball(cfg: ExperimentConfig):
    ...
    is_natural = getattr(cfg, "natural_mode", False)
    quick = bool(getattr(cfg, "quick_test", False))
    session_label = "S4 (自然态)" if is_natural else "S3 (银标准)"

    if quick:
        n_total = QUICK_N_TOTAL
        n_target = max(1, int(round(n_total * QUICK_TARGET_RATIO)))
        n_standard = n_total - n_target
        pre_stim_quiet = QUICK_PRE_STIM_QUIET
        iti_min = QUICK_ITI_MIN
        iti_max = QUICK_ITI_MAX
        behavior_check_interval = QUICK_BEHAVIOR_CHECK_INTERVAL
    else:
        n_total = N_TOTAL
        n_target = N_TARGET
        n_standard = N_STANDARD
        pre_stim_quiet = PRE_STIM_QUIET
        iti_min = ITI_MIN
        iti_max = ITI_MAX
        behavior_check_interval = BEHAVIOR_CHECK_INTERVAL
    ...
    n_forced_blinks = 0
    forced_blink_ratio = getattr(cfg, "forced_blink_ratio", 0.0)
    if is_natural and forced_blink_ratio > 0:
        target_indices = [i for i, t in enumerate(trials) if t == "target"]
        n_forced_blinks = int(len(target_indices) * forced_blink_ratio)
