...
# Quick-test (冒烟测试) 参数
QUICK_N_TRIALS_PER_FREQ = 1
QUICK_FLICKER_DURATION = 1.0
QUICK_PRE_STIM_QUIET = 0.05
QUICK_CUE_DURATION = 0.3
...
def run_ssvep(cfg: ExperimentConfig):
    ...
    if quick:
        n_trials_per_freq = QUICK_N_TRIALS_PER_FREQ
        flicker_duration = QUICK_FLICKER_DURATION
        pre_stim_quiet = QUICK_PRE_STIM_QUIET
        cue_duration = QUICK_CUE_DURATION
    else:
    ...
    n_total = n_trials_per_freq * len(FREQ_SPECS)
    ...
    if is_natural:
        instruction_text = (
            ...
            f"四个方块会同时闪烁 {flicker_duration:.0f} 秒\n"
            f"共 {n_total} 个试次\n\n"
            f"你可以完全放松，不需要控制眨眼\n"
