...
QUIET_DURATION_QUICK = 0.3
ACTION_DURATION_QUICK = 0.3
REST_DURATION_QUICK = 0.3
SACCADE_DURATION_QUICK = 0.5
QUICK_TRIALS_PER_TYPE = 2
...
def run_session2(cfg: ExperimentConfig):
    ...
    quiet_dur = QUIET_DURATION_QUICK if quick else QUIET_DURATION
    action_dur = ACTION_DURATION_QUICK if quick else ACTION_DURATION
    rest_dur = REST_DURATION_QUICK if quick else REST_DURATION
    saccade_dur = SACCADE_DURATION_QUICK if quick else SACCADE_DURATION
    ...
    for art_idx, art_type in enumerate(ARTIFACT_TYPES):
        ...
        n_trials = QUICK_TRIALS_PER_TYPE if quick else art_type["count"]
        ...
        for trial_i in range(n_trials):
            ...
            while core.getTime() - quiet_start < quiet_dur:
                draw_fixation(win)
                win.flip()
            ...
            this_action_duration = saccade_dur if is_saccade else action_dur
            ...
            while core.getTime() - rest_start < rest_dur:
    ...
    session_data = {
        ...
        "total_trials": (QUICK_TRIALS_PER_TYPE * len(ARTIFACT_TYPES)) if quick
                          else sum(a["count"] for a in ARTIFACT_TYPES),
        "quiet_duration_s": quiet_dur,
        "action_duration_s": action_dur,
        "saccade_action_duration_s": saccade_dur,
