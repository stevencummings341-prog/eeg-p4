...
```text
D:\EEG_Project\P4_EEG\experiment\data\eeg-npz\
```

EEG iRecorder 软件请把 `.bdf` 落到（与本仓库约定一致即可）：

```text
D:\EEG_Project\P4_EEG\experiment\data\eeg-bdf\
```

> [!warning]
> 整个 `experiment/data/` (含 `eeg-bdf/`、`eeg-npz/`、`video_records/`) 不进 git。真实 EEG 原始数据也不进 git。
...
```text
experiment\data\eeg-npz\
```

EEG 原始数据（`.bdf`）：iRecorder 软件保存，请配置到：

```text
experiment\data\eeg-bdf\
```

相机视频默认：

```text
experiment\data\video_records\
```
